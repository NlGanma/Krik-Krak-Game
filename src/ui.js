export function setupUI({canvas,keys}) {
  const dialog=document.querySelector('#help-dialog'),gameOver=document.querySelector('#gameover-dialog'),note=document.querySelector('#note'),hint=document.querySelector('#look-hint');
  ...
  return {paused:()=>dialog.open||gameOver.open,showNote(){clearTimeout(noteTimeout);note.classList.remove('dismissed');noteTimeout=setTimeout(()=>note.classList.add('dismissed'),16000);}};
}
